...

{
  ...
  outputs = { self, nixpkgs, nixCats, wrapper-manager, ... }@inputs: let
  ...
  forEachSystem (system: let
    ...
    wmEval = wrapper-manager.lib {
      ...
      modules = [
        {
          wrappers.nvim-wrapped = {
            basePackage = defaultPackage;
            env.XDG_CONFIG_HOME.value = pkgs.linkFarm "xdg-config" {
              "nvim" = ./.;
            };
            env.NVIM_APPNAME.value = "nvim";
          };
        }
      ];
    };
