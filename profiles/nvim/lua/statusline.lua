...
local function get_winbar_breadcrumb()
  ...
  if tail == "" then return formatted_cwd end
  local icon = ""
  local ok, mini_icons = pcall(require, "mini.icons")
  if ok then
    local filetype_icon, filetype_hl = mini_icons.get('filetype', vim.bo.filetype)
    if filetype_icon then icon = "%#" .. filetype_hl .. "#" .. filetype_icon .. " " end
  end
  ...
  local file_breadcrumb = formatted_cwd .. sep
      ...
      .. icon .. "%#Normal#" .. tail .. readonly .. modified
...
function M.setup()
  ...
  require('lualine').setup({
    ...
    winbar = { lualine_c = { { get_winbar_breadcrumb } } },
    inactive_winbar = { lualine_c = { { get_winbar_breadcrumb } } },
  })
end
