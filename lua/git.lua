...
function M.setup()
  ...
  map('v', '<leader>gs', function() gitsigns.stage_hunk({ vim.fn.line('.'), vim.fn.line('v') }) end,
    { desc = 'Stage Selection', unpack(opts) })
  map('v', '<leader>gr', function() gitsigns.reset_hunk({ vim.fn.line('.'), vim.fn.line('v') }) end,
    { desc = 'Reset Selection', unpack(opts) })
  ...
  map('n', '<leader>gh', function() gitsigns.blame_line({ full = true }) end,
    { desc = 'Blame Line', unpack(opts) })
  ...
  -- Lazygit terminal (split sensibly)
  local function split_sensibly()
    local w = vim.api.nvim_win_get_width(0)
    local h = vim.api.nvim_win_get_height(0)
    if w > math.floor(h * 2.3) then
      vim.cmd('vnew')
    else
      vim.cmd('new')
    end
  ...
  map('n', '<leader>gl', function()
    ...
    -- Put terminal at bottom/right similar to original behavior
